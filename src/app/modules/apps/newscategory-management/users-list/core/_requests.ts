...
const GET_USERS_URL = `${API_URL}/news-categories`
...
const updateUser = (id, user: User): Promise<User | undefined> => {
  return axios
    .patch(`${GET_USERS_URL}/${id}`, {
      ...user,
      ...(user.parent ? {parent: user.parent} : {parent: null}),
    })
    .then((response: AxiosResponse<Response<User>>) => response.data)
    .then((response: Response<User>) => response.data)
}
...
const getAllUsers = () => {
  return axios.get(`${GET_USERS_URL}`).then((d) => d.data.data)
}
